                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          const SizedBox(width: 12),
                          PrimaryButton(
                            label: ResponsiveHelper.isMobile(context)
                                ? 'إضافة'
                                : AppStrings.addUser,
  ...
  void _toggleUserStatus(UserModel user) async {
    ...
    try {
      ...
      await LocalLogService.instance.logActivity(
        ...
        details:
            'تم ${newStatus ? "تفعيل" : "تعطيل"} حساب المستخدم: ${updatedUser.name}',
      );
      _loadUsers();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'تم ${newStatus ? 'تفعيل' : 'تعطيل'} الحساب',
              style: const TextStyle(fontFamily: 'Cairo'),
            ),
            backgroundColor: AppColors.success,
          ),
                                    ...
                                    if (formKey.currentState?.validate() ??
                                      ...
                                      try {
                                        if (isEdit) {
                                          ...
                                          final uid = await FirebaseService
                                              .instance
                                              .registerUserAuth(
                                                email,
                                                passCtrl.text.trim(),

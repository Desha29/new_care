...
class _ProceduresScreenState extends State<ProceduresScreen> {
  ...
  Widget _hc(String text, int flex) {
    return Expanded(
      ...
      child: Text(
        text,
        style: AppTypography.tableHeader.copyWith(fontSize: 15, fontWeight: FontWeight.bold),
      ),
    );
  }
            ...
            child: Row(
              children: [
                IconActionButton.edit(
                  onPressed: () => _showProcedureDialog(item: item),
                ),
  ...
  Future<void> _showProcedureDialog({ProcedureModel? item}) async {
    ...
    final priceInsideCtrl = TextEditingController(
      text: item?.priceInside.toString() ?? '',
    );
    final priceOutsideCtrl = TextEditingController(
      text: item?.priceOutside.toString() ?? '',
    );
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            Text(
                              isEdit
                                  ? 'تعديل الخدمة/الإجراء'
                                  : 'إضافة خدمة/إجراء جديد',
                              style: const TextStyle(
                                fontFamily: 'Cairo',
                                fontSize: 20,
                                  ...
                                  if (formKey.currentState?.validate() ??
                                    ...
                                    try {
                                      final newItem = ProcedureModel(
                                        id: item?.id == null || item!.id.isEmpty
                                        ...
                                        priceInside:
                                            double.tryParse(
                                              priceInsideCtrl.text,
                                            ) ??
                                            0,
                                        priceOutside:
                                            double.tryParse(
                                              priceOutsideCtrl.text,
                                            ) ??
                                            0,
                                        defaultPrice:
                                            double.tryParse(
                                            ...
                                            0, // Fallback for old compatibility
                                      ...
                                      if (isEdit) {
                                        await FirebaseService.instance
                                            .updateProcedure(newItem);
                                      } else {
                                        await FirebaseService.instance
                                            .createProcedure(newItem);
                                      }
                                      if (mounted) {
                                        Navigator.pop(ctx);
                                        _loadProcedures();
